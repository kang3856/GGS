	...
	$.fn.fsvs = function( options ) {
		...
		var app = {
			...
			slideToIndex : function( index, e ) {
				...
				// var currentSlideIndex2 = $(".slide").data("index");
				console.log("current: "+currentSlideIndex);
				if(index==0){
					$("#allIndexVideoBox video")
						.css({"transform":"translate(-9000px,0)",
							"transition":"1s ease"}).fadeIn();
					$("#allIndexVideoBox").css("bottom","-9000px");
					$("#allIndexHeader").css("background","transparent");
				}else if(index==1){
					$("#allIndexVideoBox video")
						.attr("src","/video/custom.mp4")
						.css({"transform":"translate(300px,-200px)",
							"transition":"1s ease",
							"display":"block"});
                    $("#allIndexVideoBox").css("bottom","0");
                    $("#allIndexHeader").css("background","rgba(220,220,220,0.6)");
				}else if(index==2){
					$("#allIndexVideoBox video")
						.attr("src","/video/pickme.mp4")
						.css({"transform":"translate(800px,-200px)",
							"transition":"1s ease",
							"display":"block"});
					$("#allIndexVideoBox").css("bottom","0");
					 $("#allIndexHeader").css("background","rgba(220,220,220,0.6)");
					...
					$("#allIndexVideoBox video")
						.attr("src","/video/giftgram.mp4")
						.css({"transform":"translate(320px,-270px)",
							"transition":"1s ease",
							"display":"block"});
					$("#allIndexVideoBox").css("bottom","0");
					 $("#allIndexHeader").css("background","rgba(220,220,220,0.6)");
					console.log("data: "+$(this).data("page"));
				}
				console.log("index:"+index);
				console.log("data: "+$(".slide").data("page"));
...
(function(){
	...
	if( $( '#v8' ).length ) {
		...
		board.attr({ viewBox: [0, 0, 500, 500], height : 500, width : 500 });

		Snap.load( "/package/purposemedia/luke-snowden-front/images/svgs/v8.svg", function ( svg ) {
			var ids = ['curve','v8','notch1','notch2','notch3','notch4','hand'];
			for( var i in ids ) {
				snaps[ids[i]] = svg.select( '#' + ids[i] );
				board.append( snaps[ids[i]] );
			}
		});

		$(window).on( 'tick.v8', function(e) {
			var rotate = ( 270 / 100 ) * e.targetPercentage;
			snaps.hand.animate( { transform: "r" + rotate + ",237,260" }, e.updateFrequency, mina.linear() );
			if( rotate == 270 ) {
				snaps.notch4.attr( 'fill', '#CF000F' );
				snaps.notch3.attr( 'fill', '#DC0D1C' );
				snaps.notch2.attr( 'fill', '#E91A29' );
				snaps.notch1.attr( 'fill', '#F52635' );
			} else if( rotate >= 260 ) {
				snaps.notch4.attr( 'fill', '#747089' );
				snaps.notch3.attr( 'fill', '#DC0D1C' );
				snaps.notch2.attr( 'fill', '#E91A29' );
				snaps.notch1.attr( 'fill', '#F52635' );
			} else if( rotate >= 250 ) {
				snaps.notch4.attr( 'fill', '#747089' );
				snaps.notch3.attr( 'fill', '#747089' );
				snaps.notch2.attr( 'fill', '#E91A29' );
				snaps.notch1.attr( 'fill', '#F52635' );
			} else if( rotate >= 240 ) {
				snaps.notch4.attr( 'fill', '#747089' );
				snaps.notch3.attr( 'fill', '#747089' );
				snaps.notch2.attr( 'fill', '#747089' );
				snaps.notch1.attr( 'fill', '#F52635' );
			} else {
				snaps.notch4.attr( 'fill', '#747089' );
				snaps.notch3.attr( 'fill', '#747089' );
				snaps.notch2.attr( 'fill', '#747089' );
				snaps.notch1.attr( 'fill', '#747089' );
			}

			$('#hp').html(e.highestPercentage);
			$('#c').html(e.clickCount);
			$('#m').html(e.milliseconds);
			$('#peak').html(e.peaks);
		})
		.v8({
			updateFrequency : 100,
	...
	if( $('.bind-me.tabs').length ) {
		$('.bind-me').click(function(e){
			e.preventDefault();
			$(this).unbind( 'click' );
			$('.content')
			...
			var app = $('.content').data( 'magic-tabs' );
			...
			$('.add').click( function(e) {
				e.preventDefault();
				app.addTab( function( newTab ) {
					newTab.head = 'New Tab';
					newTab.contents = '<p>I am a new tab</p>';
				});
			});
	...
	if( $('.bind-me.accoridion').length ) {
		$('.bind-me').click(function(e){
			e.preventDefault();
			$(this).unbind( 'click' );

			$('.content').on( 'created.magic', function(e){
			    for( i in e.sections ) {
					var section = e.sections[i];
					section.heading.prepend( '<i class="fa fa-chevron-circle-down" style="margin-right:15px;"></i>' );
				}
			}).magicAccordion();
	...
	if( $('.sortable').length ) {
		var sortable = $('.menu').sortableMenu();
		$('#get-data').click( function(e){
			e.preventDefault();
			$('#sd').html( '<div>' + sortable.data('serialize') + '</div>' );
		});
	}

	ga( 'create', window.site.gta, 'auto' );
	ga( 'send', 'pageview' );
	...
	if( $('#fsvs1').length ) {
		...
		$(window).scroll(function(){ var t = $(window).scrollTop(), n = t/2; $("h1").css({ transform : "translateY("+n+"px)" });});

		$('#fsvs1').on( 'pixelTick.fsvs', function(e) {

			var fromContents = $('.contents', e.slideFrom);
			var toContents = $('.contents', e.slideTo);

			fromContents.css({  opacity : e.opacityOut });
			toContents.css({  opacity : e.opacityIn });
...
//# sourceMappingURL=all.js.map
 ...
 } catch(e) { console.log('[/package/purposemedia/luke-snowden-front/js/all.js]: ' + e.message );}
